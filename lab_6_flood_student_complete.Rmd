...
```{r}
q %>% 
  ggplot(aes(x = date, q_cfs)) +
  geom_point()

q %>% 
  ggplot(aes(x = date, w_temp)) +
  geom_point()
...
max_q <- q %>% 
  group_by(wy) %>% 
  slice_max(q_cfs, with_ties = FALSE) %>% 
  ungroup() %>% 
  filter(wy < 2023)
...
```{r}
max_q %>% 
  ggplot(aes(x = date, y = q_cfs, color = status)) +
  geom_point() +
...
max_q <- max_q %>% 
  mutate(q_rank = rank(-q_cfs)) %>% 
  select(date, wy, q_cfs, q_rank, status)
...
max_q %>% 
  ggplot(aes(y = q_cfs)) +
  geom_line(aes(x = ri_gumb), color = "Blue") +
  geom_point(aes(x = ri_gring), color = "Red") +
  labs(title = " Gringerton (red) and Gumbel (blue) estimates of flood return intervals", x = "Return interval (years)", y = "Flow (cfs)")
...
max_q %>% 
  ...
  ggplot(aes(x = ex_gumb *100, y = q_cfs)) +
  geom_point() +
...
This will substitute the 49,400 value into the last position (2022) and we would then run the analysis with that data.
